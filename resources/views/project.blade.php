<x-layouts.public :title="'XOKOL | Proyecto'">
    ...
    <section class="relative h-screen w-full flex flex-col justify-end overflow-hidden">
        ...
        <div class="absolute bottom-0 right-0 z-20 bg-primary px-8 py-6 hidden lg:flex items-center gap-12 rounded-tl-3xl">
            ...
            <form id="like-form" method="POST" action="{{ route('project.register-like', $project) }}">
                @csrf
                <button @disabled($alreadyLiked) type="submit" class="group flex items-center px-6 py-3 gap-3  rounded-xl {{ $alreadyLiked ? 'bg-neutral-900 text-primary  cursor-not-allowed' : 'bg-black  hover:bg-neutral-900 transition-all text-white' }}">
